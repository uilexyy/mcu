<template>
  <div>
    <PageHeader title="Riwayat Radiologi">
      <template #actions>
    ...
    <BaseCard v-else-if="registrations.length === 0" no-padding>
      <EmptyState title="Belum ada riwayat" description="Belum ada hasil radiologi yang tercatat." />
    </BaseCard>
    ...
    <div v-else class="space-y-4">
      <BaseCard v-for="reg in registrations" :key="reg.id">
        ...
        <div v-if="reg.result?.pdf_url" class="mt-3">
        ...
        <div class="mt-2 text-xs text-gray-400">Terakhir diperbarui: {{ formatDate(reg.updated_at || reg.created_at) }}</div>
      </BaseCard>
...
async function fetchData() {
  ...
  try {
    ...
    const res = await api.get('/radiologi/riwayat', { params })
