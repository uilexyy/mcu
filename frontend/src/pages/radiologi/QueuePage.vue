  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        <h1 class="text-2xl font-bold text-gray-800">Antrian Radiologi</h1>
      ...
      <div class="flex items-center gap-2">
        ...
        <router-link to="/radiologi/riwayat"
          class="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-pink-600 bg-pink-50 hover:bg-pink-100 rounded-lg transition-colors">
    ...
    <div v-else-if="queue.length === 0" class="bg-white rounded-xl shadow-sm p-6 text-center text-gray-500">
      Tidak ada pasien menunggu pemeriksaan radiologi.
    </div>
    ...
    <div v-else class="space-y-4">
      <div v-for="reg in queue" :key="reg.id" class="bg-white rounded-xl shadow-sm p-5">
        <div class="flex items-center justify-between">
          ...
          <router-link :to="`/radiologi/results/${reg.id}`"
...
async function fetchQueue() {
  try { const res = await api.get('/radiologi/queue'); queue.value = res.data.data }
